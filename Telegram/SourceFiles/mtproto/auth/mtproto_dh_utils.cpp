...
SecureBytes::SecureBytes(std::size_t size) : _data(size) {
}

SecureBytes::SecureBytes(bytes::vector &&data) : _data(std::move(data)) {
}
...
std::size_t SecureBytes::size() const {
	return _data.size();
}

bytes::type *SecureBytes::data() {
	return _data.data();
}

const bytes::type *SecureBytes::data() const {
	return _data.data();
}

bytes::span SecureBytes::bytes() {
	return bytes::make_span(_data);
}

bytes::const_span SecureBytes::bytes() const {
	return bytes::make_span(_data);
}

void SecureBytes::resize(std::size_t size) {
	clear();
	_data.resize(size);
}
...
bytes::type &SecureBytes::operator[](std::size_t index) {
	return _data[index];
}

const bytes::type &SecureBytes::operator[](std::size_t index) const {
	return _data[index];
}
...
SecureBytes CreateAuthKey(
	...
	return SecureBytes(BigNum::ModExp(
		first,
		BigNum(randomBytes),
		prime
	).getBytes());
