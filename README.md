...
|---|---|
| **Обход DPI** | FakeTLS‑стелс: профили отпечатка **JA4**, правдоподобные повторные TLS‑подключения через поддельные билеты возобновления, маскировка старта, фрагментация ClientHello, варьирование размера TLS‑записей, пейсинг трафика, разнос попыток подключения |
